...
function ElectionLayoutMini({ mode = "grid", active }) {
  const inner = (
    <div className={`rounded-xl p-3 transition min-h-56 ${active 
      ...
      <div className="space-y-2">
        ...
        <div className={`h-3 w-2/3 rounded ${active 
          ? "bg-neutral-200 dark:bg-neutral-700" 
          : "bg-neutral-300 dark:bg-neutral-600"
        }`} />
        <div className={`h-2 w-1/2 rounded ${active 
          ? "bg-neutral-200 dark:bg-neutral-700" 
          : "bg-neutral-300 dark:bg-neutral-600"
        }`} />
        {mode === "grid" ? (
          <div className="mt-2 grid grid-cols-3 gap-2">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={i} className={`h-8 rounded-md border-[0.5px] ${active 
                ? "bg-neutral-100 dark:bg-neutral-800 border-black/20 dark:border-white/20" 
          ...
          <label key={i} className="flex items-center gap-2 text-sm">
            {type === "radio" ? (
              ...
              <span className="inline-flex items-center justify-center h-4 w-4 rounded-[3px] border-[0.5px] border-neutral-500">
                <svg className="h-3 w-3 text-neutral-700 dark:text-white" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-7.25 7.25a1 1 0 01-1.414 0l-3-3a1 1 0 111.414-1.414L8.5 11.086l6.543-6.543a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
              </span>
            )}
...
export default function NewElectionPage() {
  ...
  function parseDMY(value) {
    ...
    if (!m) return null;
    const dd = parseInt(m[1]); const mm = parseInt(m[2]); const yyyy = parseInt(m[3]);
  ...
  function parseDMYTime12(value) {
    ...
    if (!m) return null;
    const dd = parseInt(m[1]); const mm = parseInt(m[2]); const yyyy = parseInt(m[3]);
    let hh = parseInt(m[4]); const mi = parseInt(m[5]); const suf = m[6].toUpperCase();
  ...
  function normalizeDMY(value) {
    const d = parseDMY(value); if (!d) return value;
    return toDMY(d);
  }
  function normalizeDMYTime12(value) {
    const d = parseDMYTime12(value); if (!d) return value;
    return toDMY12(d);
  }
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 4) return form.voters.length > 0;
    return true;
  }, [step, form]);
  ...
  async function submit() {
    ...
    try {
      ...
      const id = res?.electionInfo?.electionId;
      router.replace(id ? `/elections/${id}` : "/dashboard");
    } catch (e) {
  ...
  <div className="rounded-2xl border-[0.5px] border-black dark:border-white bg-white/50 dark:bg-neutral-900/30 backdrop-blur-2xl shadow-sm p-5 sm:p-6 relative overflow-hidden text-black dark:text-white">
                  ...
                  <div className="flex items-center justify-between gap-2 rounded-md border-[0.5px] border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900/30 focus-within:ring-2 focus-within:ring-neutral-900 dark:focus-within:ring-white px-2 py-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!val) { updateField("startTime", ""); return; }
                  ...
                  <div className="flex items-center justify-between gap-2 rounded-md border-[0.5px] border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900/30 focus-within:ring-2 focus-within:ring-neutral-900 dark:focus-within:ring-white px-2 py-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!val) { updateField("endTime", ""); return; }
                        const [ymd, hm] = val.split("T");
                        const [y,m,d] = ymd.split("-");
                        let [hh, mi] = hm.split(":");
                        let h = parseInt(hh,10);
          ...
          {[{ k: "radio", label: "Single vote" }, { k: "checkbox", label: "Multiple vote" }].map((p) => (
                      <button
                        type="button"
                        key={p.k}
            ...
            {step === 3 && (
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  />
                  <Button type="button" onClick={addOption} disabled={!currentOption.trim()}>Add option</Button>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.options.map((o, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 rounded-md bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
                        ...
                        <Button type="button" variant="secondary" size="sm" onClick={() => removeItem("options", idx)}>Remove</Button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {step === 4 && (
              <div>
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  />
                  <Button type="button" onClick={addVoter} disabled={!currentVoter.trim() || !/@/.test(currentVoter.trim())}>Add voter</Button>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {form.voters.map((v, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 rounded-md bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
                        ...
                        <Button type="button" variant="secondary" size="sm" onClick={() => removeItem("voters", idx)}>Remove</Button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="rounded-md border-[0.5px] border-black/20 dark:border-white/20 p-3 bg-white/50 dark:bg-neutral-900/30">
                  ...
                  <div className="space-y-1">
                    {form.options.length > 0 ? (
                      form.options.map((o, idx) => (
                        <div key={idx} className="text-inherit">{idx + 1}. {o.optionName}</div>
                      ))
                    ) : (
                ...
                <div className="rounded-md border-[0.5px] border-black/20 dark:border-white/20 p-3 bg-white/50 dark:bg-neutral-900/30">
                  ...
                  <div className="space-y-1">
                    {form.voters.length > 0 ? (
                      form.voters.map((v, idx) => (
                        <div key={idx} className="text-inherit">{idx + 1}. {v.voterEmail}</div>
                      ))
                    ) : (
          ...
          <div className="mt-6 flex items-center justify-between">
            <Button type="button" variant="secondary" disabled={step===0 || loading} onClick={()=>setStep((s)=>Math.max(0, s-1))}>Previous</Button>
            {step < steps.length - 1 ? (
              <Button type="button" disabled={!canNext || loading} onClick={()=>setStep((s)=>Math.min(steps.length-1, s+1))}>Next</Button>
            ) : (
              <Button type="button" disabled={loading} onClick={submit}>{loading?"Creating...":"Create"}</Button>
            )}
          </div>
        </div>
      </div>
